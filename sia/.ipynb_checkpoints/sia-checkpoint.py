...
from .sia_text import clip_text_l14, clip_text_b16
...
class SIA(nn.Module):
    def __init__(self,  
        ...
        #[NoObj] Token for no object/action/human
        #self.no_obj = nn.Parameter(torch.rand(1, self.embed_dim))
        #nn.init.normal_(self.no_obj, std = 0.02)
    ...
    def forward(self, video, input_ids, log_generation=None):
        ...
        outputs = self.encode_vision(image)
        text_embeds = self.encode_text(raw_text)
            
        # cat [NoObj] token
        #text_embeds = torch.cat([text_embeds, self.no_obj], dim=0)
